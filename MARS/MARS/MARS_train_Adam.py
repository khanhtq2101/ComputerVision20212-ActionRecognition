...
if __name__=="__main__":
    ...
    if opt.log == 1:
        if opt.pretrain_path != '':
            epoch_logger = Logger_MARS(os.path.join(log_path, 'PreKin_MARS_{}_{}_train_batch{}_sample{}_clip{}_nesterov{}_manualseed{}_model{}{}_ftbeginidx{}_layer{}_alpha{}.log'
                .format(opt.dataset, opt.split, opt.batch_size, opt.sample_size, opt.sample_duration, opt.nesterov, opt.manual_seed, opt.model, opt.model_depth, opt.ft_begin_index,
                                opt.output_layers[0], opt.MARS_alpha))
                            ,['epoch', 'loss', 'loss_MSE', 'loss_MARS', 'acc', 'lr'], opt.MARS_resume_path, opt.begin_epoch)
            val_logger   = Logger_MARS(os.path.join(log_path, 'PreKin_MARS_{}_{}_val_batch{}_sample{}_clip{}_nesterov{}_manualseed{}_model{}{}_ftbeginidx{}_layer{}_alpha{}.log'
                            .format(opt.dataset,opt.split,  opt.batch_size, opt.sample_size, opt.sample_duration, opt.nesterov, opt.manual_seed, opt.model, opt.model_depth, opt.ft_begin_index,
                                opt.output_layers[0], opt.MARS_alpha))
                            ,['epoch', 'loss', 'acc'], opt.MARS_resume_path, opt.begin_epoch)
        else:
            epoch_logger = Logger_MARS(os.path.join(log_path, 'MARS_{}_{}_train_batch{}_sample{}_clip{}_nesterov{}_manualseed{}_model{}{}_ftbeginidx{}_layer{}_alpha{}.log'
                            .format(opt.dataset, opt.split, opt.batch_size, opt.sample_size, opt.sample_duration, opt.nesterov, opt.manual_seed, opt.model, opt.model_depth, opt.ft_begin_index,
                                opt.output_layers[0], opt.MARS_alpha))
                            ,['epoch', 'loss', 'loss_MSE', 'loss_MARS', 'acc', 'lr'], opt.MARS_resume_path, opt.begin_epoch)
            val_logger   = Logger_MARS(os.path.join(log_path, 'MARS_{}_{}_val_batch{}_sample{}_clip{}_nesterov{}_manualseed{}_model{}{}_ftbeginidx{}_layer{}_alpha{}.log'
                            .format(opt.dataset, opt.split, opt.batch_size, opt.sample_size, opt.sample_duration, opt.nesterov, opt.manual_seed, opt.model, opt.model_depth, opt.ft_begin_index,
                                opt.output_layers[0], opt.MARS_alpha))
                            ,['epoch', 'loss', 'acc'], opt.MARS_resume_path, opt.begin_epoch)
    ...
    #tensor board
    writer_path = os.path.join(log_path, 'MARS_{}_{}_train_batch{}_sample{}_clip{}_nesterov{}_manualseed{}_model{}{}_ftbeginidx{}_layer{}_alpha{}'
                            .format(opt.dataset, opt.split, opt.batch_size, opt.sample_size, opt.sample_duration, opt.nesterov, opt.manual_seed, opt.model, opt.model_depth, opt.ft_begin_index, 
                                opt.output_layers[0], opt.MARS_alpha))
    ...
    for epoch in range(opt.begin_epoch, opt.n_epochs + 1):
        ...
        if epoch % opt.checkpoint == 0:
            if opt.pretrain_path != '':
                save_file_path = os.path.join(log_path, 'MARS_preKin_{}_{}_train_batch{}_sample{}_clip{}_nesterov{}_manualseed{}_model{}{}_ftbeginidx{}_layer{}_alpha{}_{}.pth'
                            .format(opt.dataset, opt.split, opt.batch_size, opt.sample_size, opt.sample_duration, opt.nesterov, opt.manual_seed, opt.model, opt.model_depth, opt.ft_begin_index, 
                                opt.output_layers[0], opt.MARS_alpha, epoch))
            else:
                save_file_path = os.path.join(log_path, 'MARS_{}_{}_train_batch{}_sample{}_clip{}_nesterov{}_manualseed{}_model{}{}_ftbeginidx{}_layer{}_alpha{}_{}.pth'
                            .format(opt.dataset, opt.split, opt.batch_size, opt.sample_size, opt.sample_duration, opt.nesterov, opt.manual_seed, opt.model, opt.model_depth, opt.ft_begin_index, 
                                opt.output_layers[0], opt.MARS_alpha, epoch))
            states = {
                'epoch': epoch + 1,
        ...
        writer.add_scalar('Train/Loss_Flow', losses_MSE.avg, epoch)
        writer.add_scalar('Train/Loss_Classifier', losses_MARS.avg, epoch)
